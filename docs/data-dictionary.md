...
| 17 | Organization Type | `organization_type` | `varchar(50)` | N | N | - | Y | `'본사'` | N | N | N | Y | 본사 (Headquarters) \| 공장 (Factory) \| 연구소 (Research Institute) \| 해외법인 (Overseas Subsidiary) | `본사` |
...
| 3 | Username | `username` | `varchar(50)` | N | N | - | Y | - | Y | Y | Y | Y | Login ID (duplicates not allowed) | `user.sample` |
| 4 | Password Hash | `password_hash` | `varchar(255)` | N | N | - | Y | - | N | N | Y | N | Hash used to verify the password. Do not include the hash value in responses, general exports, or audit logs. Retain Audit=N. | `[REDACTED_PASSWORD_HASH]` |
| 5 | Full Name | `full_name` | `varchar(100)` | N | N | - | Y | - | N | Y | Y | Y | User name | `Hong Gil-dong` |
| 6 | Email | `email` | `varchar(100)` | N | N | - | Y | - | Y | Y | Y | Y | Email address | `user.sample@example.com` |
...
| 10 | Last Login At | `last_login_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Timestamp of the most recent system login | `2026-08-26T16:00:00Z` |
| 11 | Password Changed At | `password_changed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Timestamp of the last password change | `2026-08-01T09:00:00Z` |
| 12 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-01T00:00:00Z` |
| 13 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T16:00:00Z` |
...
| No | Logical Column Name | Physical Column Name | Type | PK | FK | Reference | NN | Default | UQ | IDX | Sensitive | Audit | Description/Business Rules | Example Value |
|---:|---|---|---|:---:|:---:|---|:---:|---|:---:|:---:|:---:|:---:|---|---|
| 27 | Mapping ID | `user_role_id` | `uuid` | Y | N | - | Y | `gen_random_uuid()` | Y | Y | N | Y | Unique identifier for a user-to-role mapping. Apply a composite UNIQUE constraint on (user_id, role_id) to all rows in this table. There are no separate active or deletion columns. | `00000000-0000-0000-0000-000000000001` |
...
|---|---|---|---|
| `uq_user_role_user_role` | Composite UNIQUE | `user_id, role_id` | Apply composite UNIQUE to all rows |
...
| 791 | Group Code | `group_code` | `varchar(50)` | N | N | - | Y | - | N | Y | N | Y | User group identification code within the organization. Duplicate (organization_id, group_code) combinations are not allowed for rows where deleted_at IS NULL and is_active is TRUE. | `QA_REVIEWER_GROUP` |
...
| 795 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | User group creation timestamp (UTC) | `2026-09-14T09:00:00Z` |
| 796 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who created the user group | `UUID` |
| 797 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | User group last-modified timestamp (UTC) | `2026-09-14T09:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_user_group_code_active` | Conditional UNIQUE | `organization_id, group_code` | `WHERE deleted_at IS NULL AND is_active = TRUE` |
...
| 802 | User ID | `user_id` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User included in the group. Duplicate (user_group_id, user_id) combinations are not allowed for rows where deleted_at IS NULL and member_status is ACTIVE. | `UUID` |
| 803 | Member Status | `member_status` | `varchar(20)` | N | N | - | Y | `'ACTIVE'` | N | Y | N | Y | Group member status: ACTIVE, INACTIVE, WITHDRAWN | `ACTIVE` |
| 804 | Participation Started At | `joined_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Timestamp when the user joined the group | `2026-09-14T09:00:00Z` |
...
| 806 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Group membership mapping creation timestamp (UTC) | `2026-09-14T09:00:00Z` |
| 807 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who registered the group member | `UUID` |
| 808 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Group membership mapping last modification timestamp (UTC) | `2026-09-14T09:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_user_group_member_active` | Conditional UNIQUE | `user_group_id, user_id` | `WHERE deleted_at IS NULL AND member_status = 'ACTIVE'` |
...
| 815 | Project ID | `project_id` | `uuid` | N | Y | `validation_project.project_id` | N | - | N | Y | N | Y | Required when scope_type is PROJECT. NULL when scope_type is GLOBAL. A CHECK constraint validates the relationship between the scope and NULL values | `UUID` |
| 816 | Is Active | `is_active` | `boolean` | N | N | - | Y | `TRUE` | N | Y | N | Y | Whether the group role mapping is enabled. Enforce uniqueness by scope for rows where deleted_at IS NULL and is_active is TRUE. GLOBAL uses (user_group_id, role_id); PROJECT uses (user_group_id, role_id, project_id). | `TRUE` |
| 817 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Group role mapping creation timestamp (UTC) | `2026-09-14T09:00:00Z` |
| 818 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who assigned the group role | `UUID` |
| 819 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Group role mapping last modification timestamp (UTC) | `2026-09-14T09:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_group_role_global_active` | Conditional UNIQUE | `user_group_id, role_id` | `WHERE deleted_at IS NULL AND is_active = TRUE AND scope_type = 'GLOBAL'` |
| `uq_group_role_project_active` | Conditional UNIQUE | `user_group_id, role_id, project_id` | `WHERE deleted_at IS NULL AND is_active = TRUE AND scope_type = 'PROJECT'` |
| `ck_group_role_scope` | CHECK | `scope_type, project_id` | project_id must be NULL for GLOBAL and is required for PROJECT<br>`CHECK ((scope_type = 'GLOBAL' AND project_id IS NULL) OR (scope_type = 'PROJECT' AND project_id IS NOT NULL))` |
...
| Item | Definition |
|---|---|
| Description | Electronic signature evidence for document submission, review, approval, and rejection, and for project closure requests |
...
| 35 | Signature Step | `signature_action` | `varchar(50)` | N | N | - | Y | - | N | N | N | Y | SUBMIT \| REVIEW \| APPROVE \| REJECT. Record the author’s submission signature as SUBMIT. Record a project closure request signature as SUBMIT | `APPROVE` |
| 36 | Signature Purpose | `signature_meaning` | `varchar(200)` | N | N | - | Y | - | N | N | N | Y | 21 CFR Part 11 signature meaning. Store PROJECT_CLOSE_NORMAL for a normal project closure request and PROJECT_CLOSE_FORCED for a forced closure request | `Final approval of URS requirements` |
| 37 | Signature Timestamp | `signed_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Timestamp when the electronic signature was executed | `2026-08-26T16:30:00Z` |
| 38 | Target Version | `target_version` | `varchar(20)` | N | N | - | Y | - | N | N | N | Y | For documents, the display version of the revision being signed. For a validation_project closure request, store the per-project closure request sequence in the form CLOSE-1, CLOSE-2. Limit the length to 20 characters and never reuse a request version within the same project. | `v1.0` |
| 39 | Signed Content Hash | `content_hash` | `varchar(128)` | N | N | - | Y | - | N | N | N | Y | SHA-256 hash used to verify the integrity of the target document and its details at the time of signing. For a project closure request, calculate it from the UTF-8 bytes of the signed_payload string retained in the Audit Trail | `[SAMPLE_SHA256_HASH]` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_project_closure_signature` | Conditional UNIQUE | `target_record_id, target_version` | `WHERE target_table_name = 'validation_project' AND signature_action = 'SUBMIT'` |

#### Business Rules

| Item | Business Rules |
|---|---|
| Preserving the Closure Signature Input | The input JSON string for the closure signature includes project_id, the CLOSE-n version, closure_type, closure_reason, the requester, and the request timestamp. Preserve this string in Audit Trail.new_values.signed_payload and also record signature_id. content_hash is the SHA-256 hash of the UTF-8 bytes of this string. Store the request, signature, and snapshot atomically. |

Project closure request signature mapping and version assignment follow the [business rules for `validation_project`](#table-validation_project). Workflow action signature validation follows the [business rules for `approval_action`](#table-approval_action).
...
| 47 | Before Data | `old_values` | `jsonb` | N | N | - | N | - | N | N | Y | Y | JSON data before modification | `{"status": "작성중"}` |
| 48 | After Data | `new_values` | `jsonb` | N | N | - | N | - | N | N | Y | Y | JSON data after modification. A project closure request record includes signature_id and signed_payload, the input JSON string for the signature | `{"status": "승인완료"}` |
...
| 50 | Client IP Address | `client_ip` | `varchar(45)` | N | N | - | N | - | N | N | Y | Y | User's client IP address | `192.0.2.10` |
| 51 | Occurred At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Audit trail log creation timestamp (UTC) | `2026-08-26T16:30:00Z` |
...
| 55 | Target Document Version | `target_version` | `varchar(20)` | N | N | - | N | - | N | N | N | Y | For documents managed by revision, store the display version at the time of the change; NULL is allowed for ordinary tables. For project closure request and closure records, store the same CLOSE-n version as the electronic signature | `v1.0` |
...
Closure request and final closure history follow [`validation_project`](#table-validation_project); signature input retention and hashing follow the [business rules for `electronic_signature`](#table-electronic_signature).
...
| 66 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T16:30:00Z` |
...
| 68 | Expires At | `expires_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Time when the file becomes eligible for cleanup. NULL for permanent files or files subject to business retention requirements. | `2026-09-09T18:00:00Z` |
...
| 71 | Cleaned Up At | `cleaned_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when cleanup of the actual stored file and its metadata was completed | `2026-09-09T19:00:00Z` |
...
| No | Logical Column Name | Physical Column Name | Type | PK | FK | Reference | NN | Default | UQ | IDX | Sensitive | Audit | Description/Business Rules | Example Value |
|---:|---|---|---|:---:|:---:|---|:---:|---|:---:|:---:|:---:|:---:|---|---|
| 528 | Evidence Link ID | `evidence_link_id` | `uuid` | Y | N | - | Y | `gen_random_uuid()` | Y | Y | N | Y | Unique identifier for a link between a document/test item and an evidence file. Duplicate (project_id, file_id, target_entity_type, target_entity_id, evidence_type) combinations are not allowed for rows where deleted_at IS NULL. | `UUID` |
...
| 535 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Evidence link creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 536 | Author ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | ID of the user who created the evidence link | `UUID` |
| 537 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Evidence link last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_evidence_link_active` | Conditional UNIQUE | `project_id, file_id, target_entity_type, target_entity_id, evidence_type` | `WHERE deleted_at IS NULL` |
...
| 770 | Target Cutoff Time | `target_base_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Cutoff time used to find files that expired or became eligible for cleanup before this time | `2026-09-02T00:00:00Z` |
...
| 777 | Requested At | `requested_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | Y | N | Y | Time when a manual request was received or a scheduled job was registered | `2026-09-02T01:00:00Z` |
| 778 | Execution Started At | `started_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the file cleanup job actually started | `2026-09-02T01:00:05Z` |
| 779 | Execution Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the cleanup job ended in success or final failure | `2026-09-02T01:05:00Z` |
...
| 782 | Next Retry At | `next_retry_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Next scheduled execution time for a job in RETRY_WAIT status | `2026-09-02T01:15:00Z` |
...
| 785 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | File cleanup execution history creation timestamp (UTC) | `2026-09-02T01:00:00Z` |
...
| 787 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | File cleanup execution history last modification timestamp (UTC) | `2026-09-02T01:05:00Z` |
...
| 77 | System Type | `system_type` | `varchar(50)` | N | N | - | Y | - | N | N | N | Y | IT시스템 (IT System) \| 생산 장비 (Production Equipment) \| 품질 장비 (Quality Equipment) \| 유틸리티 (Utility) | `IT시스템` |
...
| 88 | Status | `status` | `varchar(20)` | N | N | - | Y | `'활성'` | N | N | N | Y | 활성 (Active) \| 검토중 (Under Review) \| 비활성 (Inactive) | `활성` |
| 89 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-25T00:00:00Z` |
| 90 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-25T00:00:00Z` |
...
| 732 | Backup Reference Time | `backup_base_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Reference time for the data to be backed up | `2026-09-02T18:00:00Z` |
...
| 737 | Requested At | `requested_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | Y | N | Y | Time when a manual request was received or a scheduled backup was registered | `2026-09-02T18:00:00Z` |
| 738 | Execution Started At | `started_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the backup job actually started | `2026-09-02T18:00:05Z` |
| 739 | Execution Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the backup job ended in success or final failure | `2026-09-02T18:20:00Z` |
...
| 742 | Next Retry At | `next_retry_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Next scheduled execution time for a job in RETRY_WAIT status | `2026-09-02T18:30:00Z` |
...
| 745 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Backup execution history creation timestamp (UTC) | `2026-09-02T18:00:00Z` |
...
| 747 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Backup execution history last modification timestamp (UTC) | `2026-09-02T18:20:00Z` |
...
| 101 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-25T00:00:00Z` |
| 102 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-25T00:00:00Z` |
...
| 108 | Status | `status` | `varchar(20)` | N | N | - | Y | `'진행 중'` | N | N | N | Y | 진행 중 (In Progress) \| 완료 (Completed) \| 강제종료 (Forced Closure) \| 보류 (On Hold) | `진행 중` |
| 109 | Start Date | `start_date` | `date` | N | N | - | Y | `CURRENT_DATE` | N | N | N | Y | Project start date | `2026-04-15` |
| 110 | Validation Approach | `validation_type` | `varchar(50)` | N | N | - | Y | `'신규 검증'` | N | N | N | Y | 신규 검증 (Initial Validation) \| 변경 검증 (Change Validation) \| 재검증 (Revalidation) | `신규 검증` |
...
| 114 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-25T00:00:00Z` |
...
| 117 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 122 | Closure Requester ID | `closure_requested_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | ID of the user who requested normal or forced closure and provided an electronic signature. Must match signer_id of the SUBMIT electronic signature for that project and closure request version | `00000000-0000-0000-0000-000000000001` |
| 123 | Closure Requested At | `closure_requested_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Timestamp when project closure was requested and the closure electronic signature was provided. Must match signed_at of that closure request signature | `2026-09-14T09:00:00Z` |
| 124 | Closed At | `closed_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Timestamp when the status was finally changed to normal or forced closure | `2026-09-14T09:05:00Z` |

#### Business Rules

| Item | Business Rules |
|---|---|
| Closure Request Signature Mapping | Record closure requests with target_table_name=validation_project, target_record_id=project_id, and signature_action=SUBMIT. signature_meaning is PROJECT_CLOSE_NORMAL for NORMAL and PROJECT_CLOSE_FORCED for FORCED. Store signer_id and signed_at unchanged in closure_requested_by and closure_requested_at, respectively. |
| Closure Request Version | For projects without a document version, assign a CLOSE-n version to each closure request (n starts at 1 and increases within each project). Within a transaction that locks the project row, assign the next number after the existing closure request signature version and never reuse it. Store it in no more than 20 characters and use a partial UNIQUE constraint to prevent duplicate request versions within the same project. |
| Final Closure Processing | Record the closure request in the Audit Trail as UPDATE with the same CLOSE-n version. At final closure, verify that the signature for that version, the preserved input, and the current closure request values match. Record final normal closure as PROJECT_CLOSE and forced closure as PROJECT_FORCE_CLOSE. If the request details have changed, require a new request version and a new signature; do not modify the existing signature or snapshot. |
...
| No | Logical Column Name | Physical Column Name | Type | PK | FK | Reference | NN | Default | UQ | IDX | Sensitive | Audit | Description/Business Rules | Example Value |
|---:|---|---|---|:---:|:---:|---|:---:|---|:---:|:---:|:---:|:---:|---|---|
| 540 | Project Member ID | `project_member_id` | `uuid` | Y | N | - | Y | `gen_random_uuid()` | Y | Y | N | Y | Unique identifier for a project member role mapping. Duplicate (project_id, user_id, role_id) combinations are not allowed for rows where deleted_at IS NULL and member_status is ACTIVE. | `UUID` |
...
| 545 | Participation Started At | `joined_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Time when project participation began | `2026-09-01T10:00:00Z` |
...
| 547 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Project participation information creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| 549 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Project participation information last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_project_member_active` | Conditional UNIQUE | `project_id, user_id, role_id` | `WHERE deleted_at IS NULL AND member_status = 'ACTIVE'` |
...
| Item | Definition |
|---|---|
| Description | Manages master data for SYSTEM_IDENTIFICATION, VP, QIA, VA, URS, FDS, DDS, DQ, FRA, IQ, OQ, PQ, RTM, and VSR activities |
...
| 553 | Activity Code | `activity_code` | `varchar(50)` | N | N | - | Y | - | Y | Y | N | Y | Activity identification code: SYSTEM_IDENTIFICATION, VP, QIA, VA, URS, FDS, DDS, DQ, FRA, IQ, OQ, PQ, RTM, VSR | `URS` |
...
| 557 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Activity master creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 558 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who registered the activity master record | `UUID` |
| 559 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Activity master last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| No | Logical Column Name | Physical Column Name | Type | PK | FK | Reference | NN | Default | UQ | IDX | Sensitive | Audit | Description/Business Rules | Example Value |
|---:|---|---|---|:---:|:---:|---|:---:|---|:---:|:---:|:---:|:---:|---|---|
| 562 | Project Activity ID | `project_activity_id` | `uuid` | Y | N | - | Y | `gen_random_uuid()` | Y | Y | N | Y | Unique project activity identifier. Duplicate (project_id, activity_id) combinations are not allowed for rows where deleted_at IS NULL. This applies regardless of whether the activity is selected for execution. | `UUID` |
...
| 567 | Activity Status | `activity_status` | `varchar(20)` | N | N | - | Y | `'LOCKED'` | N | Y | N | Y | Activity status: LOCKED, READY, IN_PROGRESS, COMPLETED, APPROVED, SKIPPED. Aggregate based on the current revision and reevaluate when deliverables are changed or revised. Do not set COMPLETED/APPROVED through simple UI input | `READY` |
| 568 | Enabled At | `activated_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the activity became READY after its prerequisites were satisfied | `2026-09-01T10:00:00Z` |
| 569 | Started At | `started_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Activity execution start time | `2026-09-01T11:00:00Z` |
| 570 | Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Activity execution completion time | `2026-09-02T15:00:00Z` |
| 571 | Approved At | `approved_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when final approval of the activity was completed | `2026-09-02T17:00:00Z` |
| 572 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Project activity creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 573 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who registered the project activity | `UUID` |
| 574 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Project activity last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_project_activity_active` | Conditional UNIQUE | `project_id, activity_id` | `WHERE deleted_at IS NULL` |

#### Business Rules

| Item | Business Rules |
|---|---|
| Current Revision and Status Aggregation | Aggregate statuses based on the current revision for the same project and activity. Map the document statuses 승인완료 (Approved) and APPROVED to the approved state; do not treat draft or review statuses as approved. Even if an approved previous revision remains, the approval condition is not satisfied when the new current revision is a draft. For activities with multiple deliverables, such as URS, check all applicable deliverables. |
...
| 579 | Predecessor Activity ID | `predecessor_activity_id` | `uuid` | N | Y | `validation_activity.activity_id` | N | - | N | Y | N | Y | Activity to check before enabling the successor activity. NULL is allowed for conditions covering all activities. Required for a STATUS condition | `UUID` |
...
| 581 | Required Status | `required_status` | `varchar(20)` | N | N | - | N | - | N | Y | N | Y | Evaluation criteria for a STATUS condition. CREATED means that a valid current deliverable exists; COMPLETED means execution is complete; APPROVED means final approval of the current deliverable. Do not store CREATED in project_activity.activity_status. Required only for STATUS conditions; follow the business rules below for detailed evaluation. | `APPROVED` |
...
| 583 | Condition Value | `condition_value` | `varchar(200)` | N | N | - | N | - | N | N | N | Y | Condition evaluation argument. For STATUS + CREATED, store an allowed target deliverable table name (e.g., requirement). Validate arguments for other conditions in the implementation of each condition; leave NULL if not needed. | - |
...
| 585 | Evaluation Order | `evaluation_order` | `integer` | N | N | - | Y | `1` | N | Y | N | Y | Evaluation order of conditions for the same successor activity. Evaluation order does not change the AND/OR relationship between conditions | `1` |
| 586 | Is Active | `is_active` | `boolean` | N | N | - | Y | `TRUE` | N | Y | N | Y | Whether the enablement condition is enabled | `True` |
| 587 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Condition creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 588 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who registered the condition | `UUID` |
| 589 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Condition last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `ck_dependency_status` | CHECK | `condition_type, predecessor_activity_id, required_status` | STATUS requires a predecessor activity and a required status. For other conditions, store NULL in required_status<br>`CHECK ((condition_type = 'STATUS' AND predecessor_activity_id IS NOT NULL AND required_status IS NOT NULL AND required_status IN ('CREATED', 'COMPLETED', 'APPROVED')) OR (condition_type <> 'STATUS' AND required_status IS NULL))` |
| `ck_dependency_created_target` | CHECK | `condition_type, required_status, condition_value` | CREATED evaluation requires a target deliverable table name. The service validates whether the table is allowed<br>`CHECK (NOT (condition_type = 'STATUS' AND required_status = 'CREATED') OR (condition_value IS NOT NULL AND btrim(condition_value) <> ''))` |

#### Business Rules

| Item | Business Rules |
|---|---|
| CREATED Evaluation | When condition_type=STATUS and required_status=CREATED, check that at least one valid current revision exists for the project and predecessor activity in the deliverable table specified by condition_value. Select rows with deleted_at set to NULL if that column exists, and is_current_version set to TRUE if that column exists. The existence of a project_activity row alone does not satisfy the condition. |
| COMPLETED / APPROVED Evaluation | COMPLETED is satisfied when the predecessor project_activity.activity_status is COMPLETED or APPROVED and the execution completion aggregation is valid. APPROVED is satisfied when the status is APPROVED and all current target deliverables have received final approval. IQ/OQ/PQ count as finally approved only when both protocol_status and record_status are APPROVED. |
| Combining Conditions and Skipping Activities | Evaluate only conditions where deleted_at IS NULL and is_active=TRUE. All REQUIRED conditions for the same successor activity must be satisfied (AND). RECOMMENDED conditions are advisory and do not block enablement. evaluation_order specifies the evaluation sequence. A SKIPPED or unselected state does not automatically satisfy CREATED/COMPLETED/APPROVED. |
| Activities Without Documents and Conditions Covering All Activities | Do not use STATUS+CREATED for activities without a separate document table, such as SYSTEM_IDENTIFICATION or VP. Evaluate them using the applicable condition type, such as context confirmation or activity selection. ALL_SELECTED_APPROVED checks that all non-deleted, selected activities in the same project, excluding the successor activity itself, have received final approval. An empty target set does not automatically satisfy the condition. |
| Validity of Condition Evaluation | Resolve the table name in condition_value through the service’s allowlist instead of inserting it directly into SQL. Treat undefined condition types, target tables, or required arguments as errors. Handle conditions other than STATUS through their respective business evaluations and leave required_status as NULL. |
...
| 140 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 159 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T10:00:00Z` |
| 160 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T10:00:00Z` |
...
| 166 | Audit Method | `audit_type` | `varchar(50)` | N | N | - | Y | `'현장 감사'` | N | N | N | Y | 현장 감사 (On-site Audit) \| 서류 감사 (Document Audit) \| 원격 감사 (Remote Audit) | `현장 감사` |
| 167 | Audit Date | `audit_date` | `date` | N | N | - | Y | `CURRENT_DATE` | N | N | N | Y | Date of audit execution (or scheduled date) | `2026-08-26` |
| 168 | Auditor | `auditor_name` | `varchar(100)` | N | N | - | N | - | N | N | Y | Y | Name of the person responsible for/conducting the audit | `Hong Gil-dong` |
| 169 | Audit Result | `audit_result` | `varchar(20)` | N | N | - | Y | `'적합'` | N | N | N | Y | 적합 (Acceptable) \| 조건부 적합 (Conditionally Acceptable) \| 부적합 (Unacceptable) | `적합` |
...
| 177 | Status | `status` | `varchar(20)` | N | N | - | Y | `'작성 중'` | N | N | N | Y | 작성 중 (Draft) \| 완료 (Completed) \| 승인완료 (Approved) | `작성 중` |
| 178 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
| 179 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 187 | Status | `status` | `varchar(20)` | N | N | - | Y | `'작성중'` | N | N | N | Y | 작성중 (Draft) \| 검토중 (Under Review) \| 승인완료 (Approved) \| 반려 (Rejected) | `작성중` |
...
| 194 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
| 195 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 204 | Document Status | `status` | `varchar(20)` | N | N | - | Y | `'작성 중'` | N | N | N | Y | Document status: 작성 중 (Draft), 검토 중 (Under Review), 승인 완료 (Approved) | `작성 중` |
| 205 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 207 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 219 | Status | `status` | `varchar(20)` | N | N | - | Y | `'작성 중'` | N | N | N | Y | Progress status: 작성 중 (Draft), 검토 중 (Under Review), 승인 완료 (Approved) | `작성 중` |
| 220 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 222 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 234 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T10:00:00Z` |
| 235 | Author ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | Identifier of the interface author | `UUID` |
| 236 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T10:00:00Z` |
...
| 247 | Document Status | `status` | `varchar(20)` | N | N | - | Y | `'작성 중'` | N | N | N | Y | Document status: 작성 중 (Draft), 검토 중 (Under Review), 승인 완료 (Approved) | `작성 중` |
| 248 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 250 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 262 | Outcome | `result_status` | `varchar(20)` | N | N | - | Y | `'PENDING'` | N | N | N | Y | Stored assessment results: PASS (Acceptable), FAIL (Unacceptable), PENDING (Pending Review). Create new items as PENDING. | `PASS` |
...
| 265 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 267 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 278 | Document Status | `status` | `varchar(20)` | N | N | - | Y | `'작성 중'` | N | N | N | Y | Document status: 작성 중 (Draft), 검토 중 (Under Review), 승인 완료 (Approved) | `작성 중` |
| 279 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 281 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 297 | Progress Status | `status` | `varchar(20)` | N | N | - | Y | `'작성 중'` | N | N | N | Y | Risk item status: 작성 중 (Draft), 검토 완료 (Review Completed) | `작성 중` |
| 298 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 300 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 313 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 315 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| Item | Business Rules |
|---|---|
| Protocol Approval Criteria | Determine whether test execution is allowed based on protocol_status=APPROVED in the current parent document. Do not allow execution based solely on item statuses. New documents and items start as DRAFT. Update the parent status and synchronize item statuses within the same revision in a single transaction. |
| Changes After Approval and Record Status | Add, modify, or delete items in an approved protocol in a new revision, starting the protocol status as DRAFT. Execution results may be recorded after protocol approval. Synchronize item record_status with the parent document and allow final record approval only when the protocol is APPROVED. |
...
| 326 | Protocol Status | `protocol_status` | `varchar(20)` | N | N | - | Y | `'DRAFT'` | N | N | N | Y | Protocol status for each item. Stored values: DRAFT (Draft), REVIEW (Under Review), APPROVED (Approved). The initial value for first creation and new revisions is DRAFT. Synchronize item statuses within the same revision based on the parent document’s protocol_status. | `APPROVED` |
| 327 | Actual Result | `actual_result` | `text` | N | N | - | N | - | N | N | N | Y | Record of actual results after test execution | `IQ Test 1 results` |
| 328 | Outcome | `qualification_result` | `varchar(20)` | N | N | - | N | - | N | N | N | Y | Final test outcome: Pass, Fail, 미실행 (Not Executed) | `Pass` |
...
| 330 | Executed At | `executed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Date and time when test execution was completed | `2026-06-22T00:00:00Z` |
| 331 | Record Status | `record_status` | `varchar(20)` | N | N | - | Y | `'DRAFT'` | N | N | N | Y | Execution record status for each item. Stored values: DRAFT (Draft), REVIEW (Under Review), APPROVED (Approved). Synchronize item statuses within the same revision based on the parent document’s record_status. Execution records may be entered only after protocol approval. | `APPROVED` |
| 332 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 334 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
Protocol approval and record status synchronization follow the [business rules for `iq_assessment`](#table-iq_assessment).
...
| 347 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 349 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
The shared IQ/OQ approval and record status synchronization rules follow the [business rules for `iq_assessment`](#table-iq_assessment).
...
| 360 | Protocol Status | `protocol_status` | `varchar(20)` | N | N | - | Y | `'DRAFT'` | N | N | N | Y | Protocol status for each item. Stored values: DRAFT (Draft), REVIEW (Under Review), APPROVED (Approved). The initial value for first creation and new revisions is DRAFT. Synchronize item statuses within the same revision based on the parent document’s protocol_status. | `APPROVED` |
| 361 | Actual Result | `actual_result` | `text` | N | N | - | N | - | N | N | N | Y | Record of actual results after test execution | `Actual result Test` |
| 362 | Outcome | `qualification_result` | `varchar(20)` | N | N | - | N | - | N | N | N | Y | Final test outcome: Pass, Fail, 미실행 (Not Executed) | `Pass` |
...
| 364 | Executed At | `executed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Date and time when test execution was completed | `2026-06-22T00:00:00Z` |
| 365 | Record Status | `record_status` | `varchar(20)` | N | N | - | Y | `'DRAFT'` | N | N | N | Y | Execution record status for each item. Stored values: DRAFT (Draft), REVIEW (Under Review), APPROVED (Approved). Synchronize item statuses within the same revision based on the parent document’s record_status. Execution records may be entered only after protocol approval. | `APPROVED` |
...
| 368 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 376 | Target PQ Completion Date | `target_completion_date` | `date` | N | N | - | N | - | N | N | N | Y | Target date for PQ completion | `2024-04-15` |
...
| 383 | Document Status | `status` | `varchar(20)` | N | N | - | Y | `'수행 대기 중'` | N | N | N | Y | PQ execution progress status. Stored values: 수행 대기 중 (Awaiting Execution), 진행 중 (In Progress), 완료 (Completed). The initial value for a new assessment is 수행 대기 중 (Awaiting Execution); this is distinct from protocol_status and record_status. | `수행 대기 중` |
...
| 386 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 388 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `ck_pq_assessment_status` | CHECK | `status` | 수행 대기 중 (Awaiting Execution), 진행 중 (In Progress), 완료 (Completed)<br>`CHECK (status IN ('수행 대기 중', '진행 중', '완료'))` |
...
| 398 | Outcome | `qualification_result` | `varchar(20)` | N | N | - | N | - | N | N | N | Y | Final test outcome: Pass, Fail, 미실행 (Not Executed) | `Pass` |
...
| 400 | Executed At | `executed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Date and time when test execution was completed | `2026-08-26T00:00:00Z` |
| 401 | Progress Status | `status` | `varchar(20)` | N | N | - | Y | `'작성중'` | N | N | N | Y | Progress status for each item: 작성중 (Draft), 승인완료 (Approved) | `작성중` |
| 402 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 404 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 420 | Document Status | `status` | `varchar(20)` | N | N | - | Y | `'작성중'` | N | N | N | Y | RTM progress status: 작성중 (Draft), 승인완료 (Approved) | `작성중` |
| 421 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 423 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 440 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 442 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 449 | Validation Conclusion | `overall_conclusion` | `varchar(100)` | N | N | - | Y | - | N | N | N | Y | Final acceptability conclusion: 적합 (Acceptable), 조건부 적합 (Conditionally Acceptable), 부적합 (Unacceptable) | `조건부 적합 (Conditionally Acceptable)` |
...
| 455 | Document Status | `status` | `varchar(20)` | N | N | - | Y | `'검토중'` | N | N | N | Y | VSR progress status: 작성중 (Draft), 검토중 (Under Review), 승인완료 (Approved) | `검토중` |
| 456 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 458 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 463 | Activity Type | `activity_code` | `varchar(20)` | N | N | - | Y | - | N | N | N | Y | Activity codes summarized in the VSR: VP, QIA, VA, URS, FDS, DDS, DQ, FRA, IQ, OQ, PQ, RTM. Manage SYSTEM_IDENTIFICATION as project context and exclude the VSR itself from detailed aggregation. | `URS` |
...
| 466 | Executed At | `execution_date` | `date` | N | N | - | N | - | N | N | N | Y | Activity execution completion date | `2024-02-15` |
...
| 470 | Conclusion/Status | `item_status` | `varchar(20)` | N | N | - | Y | `'대기'` | N | N | N | Y | Conclusion for the document/activity: 승인완료 (Approved), 검토중 (Under Review), 대기 (Pending) | `승인완료` |
| 471 | Approver Name | `approver_name` | `varchar(50)` | N | N | - | N | - | N | N | Y | Y | Full name of the final approver | `Hong Gil-dong` |
| 472 | Approval Date | `approval_date` | `date` | N | N | - | N | - | N | N | N | Y | Final approval date | `2024-02-20` |
| 473 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 475 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-26T00:00:00Z` |
...
| 483 | Submitted At | `requested_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Time when the document was submitted for review and approval | `2026-08-31T15:00:00Z` |
...
| 485 | Current Step Order | `current_step_order` | `integer` | N | N | - | Y | `1` | N | N | N | Y | Sequence number of the review/approval step currently being processed. On submission, store the step_order of the first non-deleted step; do not hard-code it as 1 | `1` |
| 486 | Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the workflow ended through final approval, rejection, or cancellation | `2026-08-31T17:00:00Z` |
| 487 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-31T15:00:00Z` |
| 488 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | N | N | Y | Creator of the workflow record | `UUID` |
| 489 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-31T16:00:00Z` |
...
| Item | Business Rules |
|---|---|
| Step Assignment for Submission and Cancellation | Create at least one non-deleted Workflow step before submission. Assign SUBMIT history to the step with the smallest step_order. Assign CANCEL during execution to the non-deleted step identified by current_step_order, and CANCEL before execution starts to the first step. Assign REVIEW/APPROVE/REJECT to the step where the action actually occurred. Do not cancel a Workflow that has ended. |
...
| No | Logical Column Name | Physical Column Name | Type | PK | FK | Reference | NN | Default | UQ | IDX | Sensitive | Audit | Description/Business Rules | Example Value |
|---:|---|---|---|:---:|:---:|---|:---:|---|:---:|:---:|:---:|:---:|---|---|
| 492 | Workflow Step ID | `workflow_step_id` | `uuid` | Y | N | - | Y | `gen_random_uuid()` | Y | Y | N | Y | Unique Workflow review/approval step identifier. Duplicate (workflow_instance_id, step_order) combinations are not allowed for rows where deleted_at IS NULL. | `UUID` |
...
| 495 | Step Type | `step_type` | `varchar(20)` | N | N | - | Y | - | N | N | N | Y | Processing step type: REVIEW or APPROVE. Record SUBMIT/CANCEL as action_type in the action history, not as step types | `REVIEW` |
...
| 499 | Due Date | `due_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Scheduled deadline for reviewing/approving this step | `2026-09-02T18:00:00Z` |
| 500 | Action Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the step was completed through approval, rejection, or another outcome | `2026-09-01T10:00:00Z` |
| 501 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-08-31T15:00:00Z` |
| 502 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | N | N | Y | Creator of the workflow step | `UUID` |
| 503 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record last modification timestamp (UTC) | `2026-08-31T16:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_workflow_step_order` | Conditional UNIQUE | `workflow_instance_id, step_order` | `WHERE deleted_at IS NULL` |

#### Business Rules

Step assignment for submission and cancellation follows [`workflow_instance`](#table-workflow_instance); actor and signature validation follow the [business rules for `approval_action`](#table-approval_action).
...
| 507 | Workflow Step ID | `workflow_step_id` | `uuid` | N | Y | `workflow_step.workflow_step_id` | Y | - | N | Y | N | Y | Workflow step to which the action history belongs. Link SUBMIT to the first non-deleted step, CANCEL to the current step in progress (or the first step before execution starts), and REVIEW/APPROVE/REJECT to the step where the action actually occurred. Do not create action history without an associated step. | `UUID` |
...
| 512 | Electronic Signature ID | `signature_id` | `uuid` | N | Y | `electronic_signature.signature_id` | N | - | N | Y | N | Y | Electronic signature record linked to the action. If a signature exists, signer_id must equal actor_id, and the target table, record, and version must match the Workflow target. For SUBMIT/REVIEW/APPROVE/REJECT, the signature action must match action_type. For CANCEL, record the cancellation reason and Audit Trail without a signature. | `UUID` |
| 513 | Action Timestamp | `acted_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | Y | N | Y | Time when submission, review, approval, or rejection was actually performed | `2026-09-01T10:00:00Z` |
| 514 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Record creation timestamp (UTC) | `2026-09-01T10:00:00Z` |

#### Business Rules

| Item | Business Rules |
|---|---|
| Submission, Cancellation, and Actor Assignment | Assigning SUBMIT/CANCEL to a step provides a link for locating the action history; it does not mean that review or approval of that step is complete. A user authorized to submit performs SUBMIT, and a user authorized to cancel performs CANCEL; record the actual actor in actor_id. For CANCEL, record the reason in action_comment and leave signature_id as NULL. Set the Workflow to CANCELLED, set remaining incomplete steps to SKIPPED, and record the Audit Trail. |
| Signature Consistency Validation | When linking an electronic signature, verify signer_id=actor_id, that the signature’s target table, record, and version match the Workflow target, and that signature_action=action_type. Also verify the hash of the signed content and successful reauthentication. Store the electronic signature link and the corresponding approval action in the same transaction. |
...
| Item | Definition |
|---|---|
| Description | Manages traceability relationships among URS, FDS, DDS, DQ, FRA, IQ, OQ, and PQ items |
...
| No | Logical Column Name | Physical Column Name | Type | PK | FK | Reference | NN | Default | UQ | IDX | Sensitive | Audit | Description/Business Rules | Example Value |
|---:|---|---|---|:---:|:---:|---|:---:|---|:---:|:---:|:---:|:---:|---|---|
| 515 | Traceability Link ID | `traceability_link_id` | `uuid` | Y | N | - | Y | `gen_random_uuid()` | Y | Y | N | Y | Unique identifier for a traceability relationship among URS, FDS, DDS, DQ, FRA, IQ, OQ, and PQ items. Duplicate (project_id, source_entity_type, source_entity_id, target_entity_type, target_entity_id, link_type) combinations are not allowed for non-deleted records. | `UUID` |
...
| 523 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Traceability link creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 524 | Author ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | ID of the user who created the traceability link | `UUID` |
| 525 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Traceability link last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| Constraint Name | Type | Applicable Columns | Condition / Rule |
|---|---|---|---|
| `uq_traceability_link_active` | Conditional UNIQUE | `project_id, source_entity_type, source_entity_id, target_entity_type, target_entity_id, link_type` | `WHERE deleted_at IS NULL` |
...
| 601 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | DDS creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 602 | Author ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who authored the DDS | `UUID` |
| 603 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | DDS last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| 613 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | DDS item creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 614 | Author ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who authored the DDS item | `UUID` |
| 615 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | DDS item last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| 628 | Resolved At | `resolved_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when deviation resolution was completed | `2026-09-03T15:00:00Z` |
| 629 | Resolved By ID | `resolved_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | User who resolved the deviation | `UUID` |
| 630 | Approved At | `approved_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Deviation closure approval timestamp | `2026-09-03T17:00:00Z` |
| 631 | Approver ID | `approved_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | User who approved deviation closure | `UUID` |
| 632 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Deviation creation timestamp (UTC) | `2026-09-01T10:00:00Z` |
| 633 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who registered the deviation | `UUID` |
| 634 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Deviation last modification timestamp (UTC) | `2026-09-01T10:00:00Z` |
...
| 642 | Reporting Period Start | `period_from` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Start date/time for querying the report's source data. NULL allowed for reports without a reporting period. | `2026-08-01T00:00:00Z` |
| 643 | Reporting Period End | `period_to` | `timestamptz` | N | N | - | N | - | N | N | N | Y | End date/time for querying the report's source data. Cannot be earlier than period_from. | `2026-08-31T23:59:59Z` |
...
| 648 | Requested At | `requested_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | Y | N | Y | Time when the report generation request was received or the batch job was registered | `2026-09-02T15:00:00Z` |
| 649 | Execution Started At | `started_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the report generation worker actually started processing | `2026-09-02T15:00:05Z` |
| 650 | Execution Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the report generation job ended in success or final failure | `2026-09-02T15:01:30Z` |
...
| 654 | Next Retry At | `next_retry_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Next scheduled execution time for a job in RETRY_WAIT status | `2026-09-02T15:10:00Z` |
...
| 657 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Report generation job record creation timestamp (UTC) | `2026-09-02T15:00:00Z` |
...
| 659 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Report generation job last modification timestamp (UTC) | `2026-09-02T15:01:30Z` |
...
| 758 | Next Run At | `next_run_at` | `timestamptz` | N | N | - | Y | - | N | Y | N | Y | Next scheduled execution time for this schedule | `2026-10-01T00:00:00Z` |
| 759 | Last Run At | `last_run_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when this schedule was last executed | `2026-09-01T00:00:00Z` |
...
| 762 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Report schedule creation timestamp (UTC) | `2026-09-02T18:00:00Z` |
| 763 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | Y | - | N | Y | N | Y | User who registered the report schedule | `UUID` |
| 764 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Report schedule last modification timestamp (UTC) | `2026-09-02T18:00:00Z` |
...
| 671 | Requested At | `requested_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | Y | N | Y | Time when the request was received | `2026-09-02T00:00:00Z` |
| 672 | Execution Started At | `started_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | AI processing start time | `2026-09-02T00:00:00Z` |
| 673 | Execution Completed At | `completed_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | AI processing completion time | `2026-09-02T00:00:00Z` |
...
| 676 | Next Retry At | `next_retry_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Scheduled retry time | `2026-09-02T15:10:00Z` |
...
| 679 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Creation timestamp | `2026-09-02T00:00:00Z` |
| 680 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | User who created the record | `UUID` |
| 681 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Modification timestamp | `2026-09-02T00:00:00Z` |
...
| 686 | Is Selected | `is_selected` | `boolean` | N | N | - | Y | `FALSE` | N | Y | N | Y | Whether adopted by the user | `True` |
| 687 | Is Applied | `is_applied` | `boolean` | N | N | - | Y | `FALSE` | N | Y | N | Y | Whether applied to the actual deliverable | `True` |
| 688 | Selected By ID | `selected_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | User who adopted the result | `UUID` |
| 689 | Selected At | `selected_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the result was adopted | `2026-09-02T00:00:00Z` |
| 690 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Creation timestamp | `2026-09-02T00:00:00Z` |
| 691 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | User who created the record | `UUID` |
| 692 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Modification timestamp | `2026-09-02T00:00:00Z` |
...
| 702 | Is Adopted | `is_selected` | `boolean` | N | N | - | Y | `FALSE` | N | Y | N | Y | Whether adopted by the user | `True` |
| 703 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Creation timestamp | `2026-09-02T00:00:00Z` |
| 704 | Created By ID | `created_by` | `uuid` | N | Y | `app_user.user_id` | N | - | N | Y | N | Y | User who created the record | `UUID` |
| 705 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Modification timestamp | `2026-09-02T00:00:00Z` |
...
| 717 | Scheduled Delivery At | `scheduled_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Scheduled notification delivery time. NULL allowed for immediate delivery. | `2026-09-02T18:00:00Z` |
| 718 | Sent At | `sent_at` | `timestamptz` | N | N | - | N | - | N | N | N | Y | Time when the notification was successfully handed off to the external delivery channel | `2026-09-02T18:00:05Z` |
...
| 721 | Next Retry At | `next_retry_at` | `timestamptz` | N | N | - | N | - | N | Y | N | Y | Next scheduled delivery time for a notification in RETRY_WAIT status | `2026-09-02T18:10:00Z` |
...
| 724 | Created At | `created_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Notification delivery job creation timestamp (UTC) | `2026-09-02T18:00:00Z` |
...
| 726 | Updated At | `updated_at` | `timestamptz` | N | N | - | Y | `CURRENT_TIMESTAMP` | N | N | N | Y | Notification delivery job last modification timestamp (UTC) | `2026-09-02T18:00:05Z` |
